...
from pyscisci.network import cocitation_network, dataframe2bipartite
...
###
def RaoStirling(data_array, distance_matrix):
    rs = 0.0
...
def compute_raostriling_interdisciplinarity(pub2ref_df, pub2field_df, focus_pub_ids=None, pub2field_norm=True, temporal=False,
    citation_direction='references', field_distance_metric='cosine', precomputed_distance_matrix=None, show_progress=False):
    """
    ...
    :param precomputed_distance_matrix : numpy array, default None
    ...
    if not precomputed_distance_matrix is None:
        if precomputed_distance_matrix != (Nfields, Nfields):
    ...
    if precomputed_distance_matrix is None:
        ...
        field2field_mat = dataframe2bipartite(df=pub2ref_df, rowname='SourceFieldId', colname='TargetFieldId',
            shape=(Nfields, Nfields), weightname='SourcePubFieldContribution').todense()

        # we no longer need the 'SourceFieldId' or 'SourcePubFieldContribution'
        ...
        precomputed_distance_matrix = pairwise_distances(field2field_mat, metric=field_distance_metric)

    # Now we start on the RaoStiring calculation
    precomputed_distance_matrix = spsparse.csr_matrix(precomputed_distance_matrix)

    # now we need to map individual publications to the rows of our matrix
    ...
    # drop any citation relationships for which we dont have field information and map the ids to int
    pub2ref_df.dropna(inplace=True)
    ...
    rsdf = 0.5 * np.squeeze(np.asarray(pub2field_mat.dot(precomputed_distance_matrix).multiply(pub2field_mat).sum(axis=1)))

    rsdf = pd.DataFrame(zip(np.sort(pub2ref_df['SourceId'].unique()), rsdf), columns = ['PublicationId', 'RaoStirling'])

    return rsdf, precomputed_distance_matrix, field2int
