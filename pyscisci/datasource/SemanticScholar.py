...
from pyscisci.datasource.readwrite import load_preprocessed_data, load_int, load_float, load_bool, download_file
from pyscisci.database import BibDataBase

semanticscholar_works_dfset = {'publications', 'references', 'publicationauthoraffiliation', 'concepts', 'fields', 'topics', 'abstracts', 'grants', 'text'}
semanticscholar_dataframe_set = {'publications', 'works', 'sources', 'authors', 'institutions', 'affiliations', 'concepts', 'fields', 'topics', 'funders'}

class SemanticScholar(BibDataBase):
    """
    Base class for the SemanticScholar interface.
    ...
    SemanticScholar comes structured into three folders: 
    ...
    def __init__(self, path2database = '', database_extension='csv.gz', keep_in_memory = False, global_filter=None, 
        ...
        self.path2embeddings = 'specter_embeddings'

        self.api_url = "https://api.semanticscholar.org/datasets/v1/release"
    ...
    def get_release_ids(self):
        """
        List currently available release dataes for Semantic Scholar files through their API.
        """
    ...
    def download_from_source(self, api_key=None, release_id = '', dataframe_list = 'all', rewrite_existing = False, show_progress=True):

        """
        Download the Semantic Scholar files through their API.
        The files will be saved to the path specified by `path2database` into compressed json files.
        ...
        dataframe_list: list
            The data types to download and save from Semantic Scholar.
                all
                abstracts
                authors
                citations
        ...
        if not os.path.exists(os.path.join(self.path2database, 'data')):
            os.mkdir(os.path.join(self.path2database, 'data'))

        if dataframe_list == 'all' or (isinstance(dataframe_list, list) and 'all' in dataframe_list):
            dataframe_list = ['papers', 'paper-ids', 'publication-venues', 'abstracts', 'authors', 'citations', 'embeddings-specter_v2','s2orc_v2', 'tldrs']


        for dataset in dataframe_list:

            path2rawdataset = os.path.join(self.path2database, 'data', dataset)
            if not os.path.exists(path2rawdataset):
                os.mkdir(path2rawdataset)

            dataset_url =  "/".join([self.api_url,release_id, 'dataset',dataset_name])
            
            file_list_response = requests.get(dataset_url, headers=headers)
            list_of_file_urls = file_list_response.json()['files']

            list_file_nameurls = [(url.split('?')[0].split('/')[-1], url) for url in list_of_files ]

            if rewrite_existing is False:
                current_files = os.listdir(path2rawdataset)
                list_file_nameurls = [(file_name, url) for file_name, url in list_file_nameurls if not file_name in current_files]


            for file_name, url in tqdm(list_file_nameurls, desc='downloading {} files'.format(dataset), leave=True, disable=not show_progress):

                download_file(url, os.path.join(path2rawdataset, file_name))
    ...
    def parse_authors(self, preprocess = True, specific_update='', process_name = True, show_progress=True):
        ...
        author_column_names = ['AuthorId', 'ORCID', 'DBLP', 'FullName', 'HomePage', 'Aliases', 'NumberPublications', 'NumberCitations', 'Hindex']
        if process_name:
        ...
        for file_name in tqdm(files_to_parse, desc='Authors', leave=True, disable=not show_progress):
            with gzip.open(os.path.join(author_dir, file_name), 'r') as infile:
                for line in infile:
                    if line != '\n'.encode():
                        aline = json.loads(line)
                        authline = [load_int(aline['authorid'])]

                        otherids = aline.get('externalids', {})
                        authline += [";".join(otherids.get(idname, None)) if isinstance(otherids.get(idname, None), list)  else None for idname in ['ORCID', 'DBLP']]

                        authline += [aline.get('name', None), aline.get('homepage', None)]
                        if isinstance(aline.get('aliases', None), list):
                            authline += [";".join(aline.get('aliases', None))]
                        else:
                            authline += [aline.get('aliases', None)]
                        
                        
                        authline += [load_int(aline.get(idname, None)) for idname in ['papercount', 'citationcount', 'hindex']]
    ...
    def parse_publications(self, preprocess = True, specific_update='', preprocess_dicts = True, start_from_file_num = 0,
        ...
        paper_dir = os.path.join(self.path2database, 'data', 'papers')

        if not os.path.exists(paper_dir):
            raise ValueError("The papers data was not found in the DataBase path.  Please download the data before parsing.")
        else:
            files_to_parse = [os.path.join(dirpath, file) for (dirpath, dirnames, filenames) in os.walk(paper_dir) for file in filenames if '.gz' in file]


        if 'all' in dataframe_list or 'papers' in dataframe_list:
            dataframe_list = semanticscholar_papers_dfset
