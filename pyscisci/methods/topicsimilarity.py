# -*- coding: utf-8 -*-
"""
.. module:: topic similarity
    :synopsis: Set of functions for finding the topical similarity of authors/journals/institutions/countries.
...
def topic_vectors(pub2source, pub2field, source_col, year_col = 'Year', count_normalize=True, pub2field_norm=False, temporal=True):
    """
    Form the topic vectors of each source (authors/journals/institutions/countries).

    Parameters
    ----------
    :param pub2source : DataFrame
        A DataFrame with the source relationships to each Publication.

    :param pub2field : DataFrame
        A DataFrame with the field relationships to each Publication.
    ...
    count_normalize : bool, default True
        If False: use the raw publication counts.
        If True: normalize each topical vector by each source

    :param pub2field_norm : bool, default False
        False = Full Counting: Each publication counts an equal amount in each field
        True = Fractional Counting: When a publication occurs in m > 1 fields, count the publication 1/m times in each field.  Normalizes the membership
        vector so it sums to 1 for each publication.

    :param temporal : bool, default False
        If True, compute the topical vectors using only publications for each year.
    ...
    pub2field['FieldInt'] = pub2field['FieldId'].map(field2int)
    ...
    if temporal:
        ...
        for y, ydf in pub2source.groupby(year_col):
            
            yfield2field_mat = field_citation_vectors(ydf, pub2field, n_fields=Nfields, count_normalize=count_normalize)

    else:

        field2field_mat = field_citation_vectors(pub2source, pub2field, n_fields=Nfields, count_normalize=count_normalize)
